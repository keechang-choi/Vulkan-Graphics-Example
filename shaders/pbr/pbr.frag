...
float DistributionGGX(vec3 N, vec3 H, float roughness) {
  ...
  float a2 =
      max(a * a, 1e-4);  // clamp: roughness=0 -> a2=0 -> NDF=0 (no specular)
...
vec3 PBR(vec3 N, vec3 V, vec3 L, vec3 radiance, vec3 albedo, float roughness,
  ...
  if (displayTargetIndex > 0) {
    switch (displayTargetIndex) {
      case 1:
        ...
        break;
      case 2:
        outFragColor.rgb = N * 0.5 + 0.5;
        break;
      case 3:
        outFragColor.rgb = albedoSample.rgb;
        break;
      case 4:
      ...
      case 7:
        ...
        break;
      case 8:
        outFragColor.rgb = emissive;
        break;
      case 9: {
        float depth = texture(samplerDepth, inUV).r;
        float sd = depth * 2.0 - 1.0;
        float ld = (2.0 * ubo.nearPlane * ubo.farPlane) /
                   (ubo.farPlane + ubo.nearPlane -
                    sd * (ubo.farPlane - ubo.nearPlane));
        ld = (ld - ubo.nearPlane) / (ubo.farPlane - ubo.nearPlane);
        ld = ld * (ubo.farPlane - ubo.nearPlane) /
             (ubo.farClamp - ubo.nearPlane);
